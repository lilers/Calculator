...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        let launchedBefore = defaults.boolForKey("firstLaunch")
    ...
    func applicationDidBecomeActive(application: UIApplication) {
        ...
        if (firstLaunch == true) {
            ...
            /*let startTime = defaults.objectForKey("start") as! NSDate
            let elapsedTime = NSDate.getTimeDifference(startTime)
            if (elapsedTime < 5)
            {*/
                let vc = ViewController()
            vc.billField.text = defaults.stringForKey("tip")
            print(defaults.stringForKey("tip"))
        }
            /*print("launch")
